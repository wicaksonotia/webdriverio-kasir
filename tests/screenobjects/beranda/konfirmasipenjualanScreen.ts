...
class KonfirmasiPenjualanScreen extends AppScreen {
    ...
    private get paymentNotes () { return $('~paymentNotes'); }
    ...
    async checkCoupon ({ nomorPonsel } : { nomorPonsel:string; }) {
        ...
        // await Gestures.checkIfDisplayedWithSwipeUp(await this.nomorPonsel, 10);
    ...
    async createOrder ({ notes } : { notes:string; }) {
        await Gestures.checkIfDisplayedWithSwipeUp(await this.paymentNotes, 10);
        await this.paymentNotes.setValue(notes);
        await Gestures.swipeUpCustom()
        await Gestures.swipeUpCustom()
